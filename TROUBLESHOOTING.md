...
- **Resolution**: Deleting a project only removes its metadata from Loop; downloaded files remain on disk and uploads stay in the remote repository. Create a new project and re-transfer the dataset files as needed.[User Guide - Projects](https://iqss.github.io/ondemand-loop/user_guide/projects/)
...
- **Resolution**: Click the pencil icon next to the project title on the project detail page to rename it instantly.[User Guide - Projects](https://iqss.github.io/ondemand-loop/user_guide/projects/)
...
- **Resolution**: Confirm the identifier matches a supported format (for example, `doi:10.xxxx/xxxx` or a full HTTPS dataset URL). Ensure the repository is supported and accessible from the current environment.【F:docs/guide/content/user_guide/finding_data.md†L1-L30】 If the dataset is draft or private, add the required API key before retrying.【F:docs/guide/content/user_guide/finding_data.md†L73-L84】
...
- **Resolution**: Loop is validated against recent Dataverse releases. Deployments that predate the 6.x series (for example, 4.20 or 5.x) are only partially supported and may break core operations.【F:docs/guide/content/user_guide/supported_repositories.md†L20-L72】 Confirm the Dataverse version with the repository administrator. If it falls in the “Partial Support” or “Testing Pending” range, document the failure, collect API responses if possible, and escalate to the Loop team. Recommend upgrading the repository to a fully supported release (6.2 or newer) to restore compatibility.【F:docs/guide/content/user_guide/supported_repositories.md†L20-L72】
...
- **Resolution**: The file selector supports keyboard navigation for browsing directories and staging files. Users can select entries using arrow keys and activate them with Enter instead of drag and drop.【F:docs/guide/content/user_guide/upload_file_selector.md†L1-L49】
...
- **Resolution**: Advise the user to double-click files in the selector to stage them or rely on keyboard navigation, both of which are fully supported alternatives.【F:docs/guide/content/user_guide/upload_file_selector.md†L11-L38】
...
- **Diagnosis**: Loop stores its state on disk. If the metadata directory becomes unavailable or read-only, the application cannot persist project definitions, repository settings, or transfer queues. Similarly, if the download workspace is missing, newly scheduled downloads will fail immediately.【F:docs/guide/content/user_guide/projects.md†L65-L112】【F:docs/guide/content/admin/configuration.md†L33-L76】
- **Resolution**: Verify that the paths configured for `metadata_root` and `download_root` exist and are writable by the affected user account. Restore the storage mount or adjust filesystem permissions as needed. Restart the Loop Passenger app (or the container) after the storage is restored so cached configuration is reloaded. Ask the user to refresh the browser and retry the action. If the directories were deleted, recreate them and copy any available backups of the `.loop_metadata` tree before restarting services.【F:docs/guide/content/admin/configuration.md†L33-L76】
...
- **Resolution**: Loop does not automatically retry failed transfers. Cancelled or failed downloads must be re-added from the dataset listing.【F:docs/guide/content/user_guide/downloading_files.md†L156-L188】 For uploads, reopen the relevant upload bundle and use **Add Files** to restage the local file so it re-enters the queue.【F:docs/guide/content/user_guide/uploading_files.md†L1-L88】 Review the **Events** log on the file to understand the failure reason before retrying.【F:docs/guide/content/user_guide/uploading_files.md†L114-L152】【F:docs/guide/content/user_guide/downloading_files.md†L120-L151】
...
- **Resolution**: Check the **Job Status** icon on the global Downloads or Uploads page. A pause icon indicates that no background worker is running.【F:docs/guide/content/user_guide/downloading_files.md†L96-L135】【F:docs/guide/content/user_guide/uploading_files.md†L124-L180】 If paused, an administrator should restart the Loop application or background job service so transfers can resume. Verify that the metadata and download directories are writable; storage outages can also block workers.【F:docs/guide/content/admin/configuration.md†L33-L112】
- **Escalation**: If jobs remain paused after a restart, escalate with logs from the background worker and filesystem health checks.